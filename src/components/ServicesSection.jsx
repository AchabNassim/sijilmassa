...
const services = [
  {
    icon: (
      <FaPlane
        size={48}
        className="text-[color:var(--color-gold-500)] mb-4"
      />
    ),
    name: "الرحلات الجوية",
  },
...
const ServicesSection = () => (
  ...
  >
    <h2 className="text-3xl md:text-4xl font-extrabold text-center mb-8 text-[color:var(--color-green-500)]">
      نحن نقدّم أعلى جودة ممكنة في كل خدمة من خدماتنا
    </h2>
    <div className="flex flex-row flex-wrap items-center md:flex-row gap-10 w-full max-w-4xl justify-center xl:items-stretch">
      {services.map((service) => (
        ...
        >
          {service.icon}
          <h3 className="text-xl font-bold text-center text-[color:var(--color-gold-500)]">
            {service.name}
          </h3>
        </div>
      ))}
